...
if [ ! -f "${OUTPUT_DIR_DATA}/dev/dev/dev.fr" ]; then
  ...
  echo "${OUTPUT_DIR_DATA}/dev/dev/dev.en and ${OUTPUT_DIR_DATA}/dev/dev/dev.fr already exist!"
fi

# Convert newstest2014 data into raw text format
if [ ! -f "${OUTPUT_DIR_DATA}/test/test-full/test.fr" ]; then
  ${OUTPUT_DIR}/mosesdecoder/scripts/ems/support/input-from-sgm.perl \
  ...
  echo "${OUTPUT_DIR_DATA}/test/test-full/test.en and ${OUTPUT_DIR_DATA}/test/test-full/test.fr already exist!"
fi

# Copy dev/test data to output dir
if [ ! -f "${OUTPUT_DIR}/test.en" ]; then
